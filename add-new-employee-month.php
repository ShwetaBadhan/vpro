...
if (isset($_POST['submit'])) {
    $emp_name       = $_POST['employee_id'];   // employee_id (int)
    $emp_designation = $_POST['designation'];  // designation (string)
    $month          = $_POST['month_year'];
    $status         = $_POST['status'];
    $created_at     = date('Y-m-d H:i:s');

    // === EMPLOYEE IMAGE UPLOAD ===
    $emp_img_path = ""; // always use this variable
    ...
    if (!empty($_FILES['template_image']['name'])) {
        ...
        $template_img_path = "assets/images/template/employee/" . time() . "_" . basename($template_img);
        move_uploaded_file($template_tmp, $template_img_path);

        // Overwrite default template if needed
        copy($template_img_path, "assets/images/template/employee/default_template.jpg");
    } else {
        $template_img_path = "assets/images/template/employee/template.jpg"; // fallback
    }

    // === DATABASE INSERTION ===
    $stmt = $db->prepare("INSERT INTO emp_month 
        (employee_id, designation, month_year, employee_image, template_image, status, created_at) 
        VALUES (?, ?, ?, ?, ?, ?, ?)");

    // employee_id = int, others are strings (except status, depends if varchar or int)
    $stmt->bind_param("issssss", $emp_name, $emp_designation, $month, $emp_img_path, $template_img_path, $status, $created_at);

    if ($stmt->execute()) {
        $_SESSION['msg'] = "<div class='alert alert-success'>Employee of the Month added successfully.</div>";
...
// personal_id, name FROM personal_details";
...
<head>
    <title>Add New Employee of the Month</title>
    <meta charset="utf-8">
                    ...
                    <div class="row align-items-center">
                        <div class="col-md-12">
                            <div class="page-header-title">
                                <h5 class="m-b-10">Add New Employee of the Month</h5>
                            </div>
                        </div>
                    </div>
                </div>
                            ...
                            <form method="post" action="" enctype="multipart/form-data" autocomplete="off">
                                ...
                                <div class="row">
                                    ...
                                    <div class="col-md-6">
                                        ...
                                        $defaultImagePath = 'assets/images/template/employee/default_template.jpg';

                                        // Query to fetch the most recent uploaded template image
                                        $selectExistingImagesQuery = "SELECT template_image FROM emp_month WHERE template_image IS NOT NULL AND template_image != '' ORDER BY month_id DESC LIMIT 1";
    ...
    ?>
                                    ...
                                    <div class="col-md-12 mt-3">
                                        <button type="submit" class="btn btn-secondary" name="submit">
                                            <i class="feather icon-save"></i> Add Employee of the Month
                                        </button>
                                    </div>
                                </div>
                            </form>
