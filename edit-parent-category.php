...
// Fetch existing Parent Categories details for update
if (isset($_GET['id'])) {
    $encodedparent = $_GET['id'];
    $parentcategory = base64_decode($encodedparent);

    // Validate $parentcategory to ensure it's numeric and valid
    if (!is_numeric($parentcategory)) {
        echo "Invalid Parent Categories ID!";
        exit;
    }

    // Corrected query to fetch the Parent Categories details
    $query = "SELECT * FROM parent_category Where id = $parentcategory";
    $result = mysqli_query($db, $query);

    if ($result && mysqli_num_rows($result) > 0) {
        $row = mysqli_fetch_assoc($result);

        // Existing Parent Categories details
        $existingName = $row['name'];
        $existingStatus = $row['status'];
    } else {
        echo "Parent Categories not found!";
        exit;
    }
} else {
    // For adding new Parent Categoriess
    $existingName = "";
    $existingStatus = "";
}

// Add or update Parent Categories
if (isset($_POST['submit'])) {
    $parentname = $_POST['name'];
    $status = $_POST['status'];

    // Update or insert Parent Categories details
    if (isset($_GET['id'])) {
        // Update existing Parent Categories
        $updateQuery = "UPDATE parent_category 
                        SET Parent name = '$parentname', status = '$status'
                        WHERE Parent Categories_id = $parentcategory";
    } else {
        // Insert new Parent Categories
        $updateQuery = "INSERT INTO parent_category (name, status)
                        VALUES ('$parentname', '$status')";
    }
    ...
    if (!empty($updateQuery)) {
        ...
        if ($updateResult) {
            ...
            echo ("<SCRIPT LANGUAGE='JavaScript'>window.location.href='manage-parent-categories.php?status=$encodedStatus';</SCRIPT>");
        } else {
                    ...
                    <div class="row align-items-center">
                        <div class="col-md-12">
                            <div class="page-header-title">
                                <h5 class="m-b-10"><?php echo isset($_GET['id']) ? 'Update' : 'Add'; ?> Parent Categoriess
                                </h5>
                            </div>
<!--                             <ul class="breadcrumb"> -->
                                ...
                                <div class=" ">
                                    ...
                                    <div class="row">
                                        <div class="col-xl- col-lg-6 col-md-12 col-sm-12 col-12">
                                            <div class="form-group"> Parent Categories <span class="red-text">*</span>
                                                <label class="sr-only control-label" for="Parent name"> Parent Categories<span class=" ">
                                                        </span></label>
                                        ...
                                        <div class="col-xl-12 col-lg-12 col-md-12 col-sm-12 col-12">
                                            <button type="submit" class="btn btn-secondary" name="submit"
                                                id="submit">
                                                <i class="feather icon-save"></i>&nbsp; <?php echo isset($_GET['id']) ? 'Update' : 'Add'; ?> Parent Categories
                                            </button>
                                        </div>
                                    </div>
                                </div>
