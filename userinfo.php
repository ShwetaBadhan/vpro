...
class UserInfo{
	...
	public static function get_ip() {
		...
		if (strpos($mainIp, ',') !== false) {
			$mainIp = trim(explode(',', $mainIp)[0]);
		}
	...
	// NEW METHOD: Get country using ip-api.com
	public static function get_country($ip = null) {
		$ip = $ip ?: self::get_ip();
		...
		if ($response) {
			...
			if (isset($data['status']) && $data['status'] === 'success') {
				return $data['country']; // you can also return $data for more info
			}
		}
		return 'Unknown';
	}
